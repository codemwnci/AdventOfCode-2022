...
class Day04 {
    ...
    private fun getRangePairs() = file.readLines().map { lines ->
    ...
    fun puzzle1() {
        println(getRangePairs().count { it[0].containsAll(it[1]) || it[1].containsAll(it[0]) })
    }

    fun puzzle2() {
        println(getRangePairs().count { it[0].intersect(it[1]).isNotEmpty() })
    }
}